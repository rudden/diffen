<template>
    <div>
        ...
        <template v-if="showThreadSelection">
            <div class="card mt-3">
                <div class="card-body">
                    <div class="row">
                        <div class="col">
...
import { GET_FILTER, GET_THREADS, CREATE_POST, UPDATE_POST, FETCH_PAGED_POSTS, FETCH_THREADS, SET_IS_LOADING_POSTS, SET_SHOULD_RELOAD_POST_STREAM } from '../../modules/forum/types'
...
export default class NewPost extends Vue {
    ...
    get allThreads() {
        return this.threads.concat(this.newThreads)
    }
    get threadsBtnText() {
    ...
    get threadsTooltip() {
        return this.selectedThreads.length <= 0 ? 'Lägg till en tråd' : `Taggat med: ${this.selectedThreads.map((t: Thread) => t.name.toLowerCase()).join(', ')}`
    }
    ...
    submit() {
        ...
        this.newPost.newThreadNames = this.newThreads.length > 0 ? this.newThreads.map((t: Thread) => t.name) : undefined
    ...
    addToSelections() {
        ...
        let thread: Thread = { id: 0, name: this.newThreadName.toLowerCase(), numberOfPosts: 0 }
